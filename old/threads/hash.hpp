...
#ifndef DEBUG2
#ifdef DEBUG
  printf("Change-Element Key %s Pos %d Value %llu\n",key.getValue(),pos,value);
#endif
...
int Hash<T>::insertElement(Pstring key, T value)
{
  ...
  int pos=hashFunction(key);
#ifdef DEBUG
  printf("Insert-Element Key %s Pos %d Value %llu\n",key.getValue(),pos,value);
#endif
  // Element to be changed
...
#else
  printf("Set-Value Key %s Value %llu\n",key.getValue(),value);
  return (changeElement(key,value)==0)||(insertElement(key,value)==0);
#endif
